...
func TestResources1(t *testing.T) {
	...
	{
		res := makeRes("exec", "x1")
		r := res.(*ExecRes) // if this panics, the test will panic
		s := "hello, world"
		f := "/tmp/whatever"
		r.Cmd = fmt.Sprintf("echo '%s' > '%s'", s, f)
		r.Shell = "/bin/bash"
		r.IfCmd = "! diff <(cat /tmp/whatever) <(echo hello, world)"
		r.IfShell = "/bin/bash"
		r.WatchCmd = fmt.Sprintf("/usr/bin/inotifywait -e modify -m %s", f)
		r.WatchShell = "/bin/bash"
		...
		timeline := []Step{
			...
			NewChangedStep(1000*60*5, false), // did we do something?
			fileExpect(f, s+"\n"),            // check again
			sleep(1),                         // we can sleep too!
		}
